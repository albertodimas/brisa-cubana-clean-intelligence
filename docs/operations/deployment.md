...
| API                                         | `JWT_SECRET`, `AUTH_SECRET`                                                                                                                                 | All                 | Deben coincidir con seeds y suites de Playwright                                                                                                                                                                    |
| API                                         | `ALLOWED_ORIGINS`, `LOGIN_RATE_LIMIT`, `LOGIN_RATE_LIMIT_WINDOW_MS`                                                                                         | All                 | Valores documentados en [`operations/security.md`](security.md). **Importante:** la API no arranca sin `ALLOWED_ORIGINS`; verifica que esté definido en Vercel (development/preview/production) antes de desplegar. |
| API/Stripe                                  | `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`                                                                                                                | All                 | Modo test en Preview/Dev, modo live en Production                                                                                                                                                                   |
| API (Portal cliente)                        | `PORTAL_MAGIC_LINK_FROM`, `PORTAL_MAGIC_LINK_SMTP_*`, `PORTAL_MAGIC_LINK_BASE_URL`, `PORTAL_MAGIC_LINK_CONFIRMATION_PATH`, `PORTAL_MAGIC_LINK_EXPOSE_DEBUG` | Production, Preview | SMTP obligatorio; deja `PORTAL_MAGIC_LINK_EXPOSE_DEBUG` vacío/`false` fuera de QA local.                                                                                                                            |
...
| Web                                         | `PROXY_ALLOWED_ORIGINS`                                                                                                                                     | All                 | Lista de orígenes permitidos (coma). Debe incluir dominios oficiales del panel/portal.                                                                                                                              |
...
| Secreto                                                                                      | Uso                                                                   |
| -------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| `JWT_SECRET`, `AUTH_SECRET`                                                                  | Seeds y autenticación en suites Playwright.                           |
...
3. Los pull requests usan `.github/workflows/vercel-preview.yml`, que ejecuta lint/type/tests y valida los builds locales (`pnpm build`). Para la **Web** seguimos usando `vercel build` + `vercel deploy --prebuilt --yes` (ahorra tiempo cuando los assets ya existen). Para la **API** ahora mandamos el código fuente directamente con `vercel deploy --yes --force` desde `apps/api`, de forma que Vercel reinstala dependencias y empaqueta la función serverless. El handler reside en `apps/api/api/index.js` (re-exporta `dist/index.js`), así que el runtime incluye `hono`, Prisma, etc., sin depender de la autodetección de Next.js. Los pushes a `main` continúan dejando que Vercel realice el build remoto tras validar localmente con Turbo.
...
- Requiere configurar los secretos en GitHub:
  ...
  - `JWT_SECRET`, `AUTH_SECRET` para mantener coherencia con los seeds.
