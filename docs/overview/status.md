...
- Portal cliente `/clientes` exhibe landing beta moderna y CTA doble (demo + contacto) y dashboard `/clientes/[customerId]` read-only con métricas, timeline y callout de expiración de sesión (contador visible); funcionalidades de autoservicio siguen planificadas (ver RFC §8).
