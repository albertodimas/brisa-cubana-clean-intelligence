...
- Despliegues en Vercel: el run `CI (Main Branch)` #18797700488 (25-oct-2025 03:43 UTC) ahora falla al intentar `vercel deploy apps/api --prod` porque la CLI concatena `apps/api/apps/api` (Root Directory configurado en Vercel). Sigue pendiente ajustar ese valor o volver al prebuild manual mientras Vercel publica un fix. El proceso manual mediante `scripts/manual-vercel-deploy.sh` continúa siendo el camino seguro para producción hasta nuevo aviso.
