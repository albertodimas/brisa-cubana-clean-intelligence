...
- SMTP productivo configurado con SendGrid (`smtp.sendgrid.net`, puerto 465) con dominio `brisacubanacleanintelligence.com` autenticado (DKIM + SPF) y pruebas manuales exitosas (`POST /api/portal/auth/request` devuelve 200).
- Alias `cliente@brisacubanacleanintelligence.com` reenviado via ImprovMX → `operaciones@` para smoke tests del portal cliente (ver `docs/operations/email-routing.md`).
