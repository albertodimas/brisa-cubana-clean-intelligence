...
| **Fase 4** (en progreso)                    | Zod 4.x                    | Migrar validaciones reemplazando `required_error`, revisar `safeParse` y tipos. Ejecutar test suite + contract tests.                         |
...
1. Completar Fase 4 (Zod 4.x) en `upgrade/zod-4` y fusionar tras validaciones.
2. Evaluar si los contratos documentados (OpenAPI y docs) requieren ajustes tras Zod 4.x.
