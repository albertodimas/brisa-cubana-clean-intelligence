...
async function main() {
  ...
  for (const file of files) {
    ...
    try {
      const { stdout, stderr } = await execa('npx', ['mmdc', '-i', file, '-o', target], {
        stdout: 'pipe',
        stderr: 'pipe'
      });
      if (stdout) {
      ...
      if (errorText.includes('chromium')) {
        console.warn('[diagrams] Mermaid CLI requires a Chromium binary. Run `pnpm approve-builds puppeteer` or install a system Chromium and set PUPPETEER_EXECUTABLE_PATH. Skipping diagram render.');
        return;
      }
      throw error;
    }
