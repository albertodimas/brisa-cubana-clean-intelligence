...
function normalize(params: UtmParams | null | undefined): UtmParams | null {
  ...
  const entries: Array<[keyof UtmParams, string]> = [];
...
export function extractUtmFromSearch(search: string): UtmParams | null {
  ...
  for (const key of UTM_KEYS) {
    const value = sanitize(query.get(`utm_${key}`));
    if (value) {
      collected[key] = value;
    }
  }
...
export function storeUtm(
  ...
  try {
    ...
    // El almacenamiento puede estar lleno o restringido; lo ignoramos silenciosamente.
  }
}

export function resolveAndStoreUtm(
  search: string,
  storage: StorageLike | null = getStorage(),
): UtmParams | null {
  ...
  if (hasUtm(fromSearch)) {
    storeUtm(fromSearch, storage);
    return fromSearch;
  }

  return loadStoredUtm(storage);
}
